...
async function obtenerExamenPorID(id) {
    ...
    return docSnap.exists() ? docSnap.data() : null; // Devuelve null si no existe
}
// Función para llenar los inputs del formulario con los datos del examen
function llenarFormularioConExamen(examen) {
    document.getElementById("txtTit").value = examen.titulo;
    document.getElementById("txtFecha").value = examen.fechaFinalizacion;
    document.getElementById("txtHora").value = examen.horaFinalizacion;
    document.getElementById("txtTempoHoras").value = examen.tiempoDuracion.horas;
    document.getElementById("txtTempoMins").value = examen.tiempoDuracion.minutos;
    document.getElementById("txtNumIntentos").value = examen.numeroIntentos;

    // Actualizar los campos para que Materialize reconozca los cambios
    M.updateTextFields();
}

// Función para obtener el ID del examen y cargar los datos
document.addEventListener('DOMContentLoaded', async () => {
    const idExamen = obtenerID();
    if (idExamen) {
        const examen = await obtenerExamenPorID(idExamen);
        if (examen) {
            llenarFormularioConExamen(examen); // Llama a la función para llenar el formulario
        }
    }
});
...
async function actualizarDatosPregunta() {
    try {
        ...
        // modal1.close(); 
        notificar("¡Pregunta actualizada correctamente!");
        restaurarOverflow();
...
document.getElementById('btnSubirEx').addEventListener('click', async function(event) {
    ...
    const idExamen = obtenerID(); // Suponiendo que esta función te devuelve la ID del examen que deseas actualizar
    ...
    try {
        // Referencia al documento del examen que se va a actualizar
        const examenRef = doc(db, "Examenes", idExamen); // Obtén la referencia del examen por su ID

        // Actualizar el examen en Firebase
        ...
        console.log("Examen actualizado con ID: ", idExamen);
...
// Agregar evento para el botón de aceptar en el modal de confirmación
